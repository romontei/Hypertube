...
.then(response => {
    ...
    .then(bdd => {
	    ...
	    var api_key = "xxxx"
            var url_rq = `https://api.themoviedb.org/3/search/movie?api_key=${api_key}&language=en-US&query=${title}&page=1&include_adult=false&year=${year}`
            function noEchec(url_rq) {
                var done = false
                var i = 1
                getJSON(url_rq, (err, res) => {
                    fs.appendFileSync('movies.json', `https://api.themoviedb.org/3/movie/${res.results[0].id}?api_key=${api_key}&language=en-US#${title}`+ '\n')
                    done = true
                    x++
